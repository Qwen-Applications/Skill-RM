"""Pure helper functions preloaded into the IF execute_python tool.

The `execute_python` environment already provides `prompt`, `instruction`,
`response`, `system_prompt`, `history`, `checklist`, and `sample`. These
helpers intentionally avoid file, network, subprocess, or external-package
access.
"""
